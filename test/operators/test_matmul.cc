...
TEST(Matmul, ShapeInference) {
  ...
  {
    Graph g = make_ref<GraphObj>(runtime);
    auto A = g->addTensor(Shape{1, 3, 2, 5});
    auto B = g->addTensor(Shape{2, 3, 5, 4});
    auto matmul = g->addOp<MatmulObj>(A, B, nullptr, false, false);
    auto C = matmul->getOutputs()[0];
    EXPECT_EQ(C->getDims(), (Shape{2, 3, 2, 4}));
  }
}
